...
for($i = 0;$i<$numerodedatos;$i++){
    ...
    if( $angulomedio < 90 || $angulomedio > 270){
        imageline($imagen, 
              ...
              200+sin(deg2rad($angulomedio))*180, 
              $color);
        imagettftext($imagen, 14, 0, 610, 200+sin(deg2rad($angulomedio))*180, $color, $fuente, $series[$i]." - ".$datos[$i]);
    }
    if( $angulomedio > 90 && $angulomedio < 270){
        $dimensiones = imagettfbbox(14, 0, $fuente, $series[$i]." - ".$datos[$i]);
        ...
        imageline($imagen, 
              ...
              200+sin(deg2rad($angulomedio))*180, 
              $color);
        imagettftext($imagen, 14, 0, 190-$anchura, 200+sin(deg2rad($angulomedio))*180, $color, $fuente, $series[$i]." - ".$datos[$i]);
    }
}
imageline($imagen, 
              400, 
