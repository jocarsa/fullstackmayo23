...
echo "Actualizando un elemento existente en: ".$_GET['tabla'];
                ...
                echo '<form method="POST" action="?tabla='.$_GET['tabla'].'&operacion=procesaactualizar"  enctype="multipart/form-data">';
                $resultado = mysqli_query($mysqli, $peticion);
                while ($fila = mysqli_fetch_assoc($resultado)) { 
                    ...
                    if(strpos($fila['Field'], "FK") !== false){
                        ...
                        echo '</select>';
                    }else if(strpos($fila['Field'], "imagen") !== false){
                        echo "<img src='foto/".$intermediario."'>";
